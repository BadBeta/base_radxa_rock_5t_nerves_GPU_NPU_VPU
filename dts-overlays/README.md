...
| `rock5t-display-dsi.dts` | MIPI DSI panel on DSI0 |
...
```bash
dtc -@ -I dts -O dtb -o rock5t-camera-imx219.dtbo rock5t-camera-imx219.dts
```
...
2. In U-Boot, load and apply the overlay before booting:
   ...
   ```
   fatload mmc 0:1 ${fdt_addr} rk3588-rock-5t.dtb
   fdt addr ${fdt_addr}
   fatload mmc 0:1 0x10000000 rock5t-camera-imx219.dtbo
...
3. For Nerves, the simplest approach is to merge the overlay nodes
   ...
   (`patches/linux/0001-arm64-dts-rockchip-rock-5t-*.patch`) and
   rebuild.
