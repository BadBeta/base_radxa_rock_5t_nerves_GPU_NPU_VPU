...
# post-build.sh - Post-build script for Rock 5T Nerves system
#
...
# Print build info
echo "Running Rock 5T post-build script..."
...
if [ -f "${KIOSK_CERT}" ] && [ -f "${CA_BUNDLE}" ]; then
    echo "" >> "${CA_BUNDLE}"
    echo "# Rock5T Kiosk localhost self-signed cert" >> "${CA_BUNDLE}"
...
echo "Rock 5T post-build script completed."
